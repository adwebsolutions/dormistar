...
if ( !function_exists( 'inspiry_register_custom_extension_loader' ) ) :
    ...
    add_action("redux/extensions/redux_demo/before", 'inspiry_register_custom_extension_loader', 0);
endif;
...
if ( !function_exists( 'wbc_importer_description_text' ) ) {
    function wbc_importer_description_text( $description ) {
        $message =  '<p>'. esc_html__( 'Only use demo importer on new WordPress install.', 'framework' ) . ' ' .
            esc_html__( 'Imported images are only for demo purpose.', 'framework' ) . ' ' .
            esc_html__( 'Import process can take few minutes to complete so please be patient and wait for its completion.', 'framework' ) .'</p>';
        return $message;
    }
    ...
    function inspiry_after_import_settings( $demo_active_import , $demo_directory_path ) {
        ...
        if ( isset( $demo_active_import[$current_key]['directory'] ) &&
            ...
            if ( isset( $footer_menu->term_id ) ) {
                    $menu_locations['footer-menu'] = $footer_menu->term_id;
            }
